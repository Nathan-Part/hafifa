...
export function validateGroup(obj: GroupInterface, except?: string) 
{  
...
export const deleteGroupFather = async(idGroup: string) => 
{
  await GroupModel.updateOne(
      { _id: idGroup },
      { "$unset": { "groupFather": "" } }
...
export const getMergedChildGroups = async(id: string) => 
{
    ...
    const inGroupsForParent = await PersonInGroupModel.find({ groupId: id });
    const personsForParent = [];
    for await (const inGroup of inGroupsForParent) {
      const person = await PersonModel.findById(inGroup.personId);
      if (person) {
