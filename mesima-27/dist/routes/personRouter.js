...
personRouter.get("/", async (req, res) => {
    try {
        ...
        const result = await Promise.all(users.map(async (user) => {
            const memberships = await models_1.PersonInGroupModel.find({
                personId: user._id,
...
personRouter.get("/deleteFromGroup/:idPerson/:idGroup", async (req, res) => {
    try {
        const idPerson = req.params.idPerson;
        const idGroup = req.params.idGroup;
        (0, personInGroupControllers_1.deletePersonInGroup)(idPerson, idGroup);
