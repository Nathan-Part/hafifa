...
import { addUser, deleteUser, editUser, getUsers, validateObj, verifId } from '../controllers/personControllers';
import { addUserInGroup, checkIfExist, findGroupById, getGroupsDetails } from '../controllers/personInGroupControllers';
...
personRouter.get('/', async (req: Request, res) => {
    try {
      ...
      {
        const memberships = await PersonInGroupModel.find({ personId: user._id }).populate('groupId', null, 'Group');
