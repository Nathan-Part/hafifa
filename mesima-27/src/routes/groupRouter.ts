...
groupRouter.post("/create", async (req: Request, res) => {
  try {
    ...
    if (newGroup.isValide) {
      ...
      const cGroup = await addGroup(newGroup.object);
      res.send(cGroup);
    } else {
...
groupRouter.get("/addPerson/:personId/:groupId", async (req: Request, res) => {
  try {
    const personId = new ObjectId(req.params.personId);
  ...
  try {
    ...
    if (await verifId(id)) {
      ...
      if (!childIds.includes(groupFather)) {
        const newGroup = validateGroup(groupData, "name");
        if (newGroup.isValide) {
          console.log(newGroup.object);
...
groupRouter.get("/deleteFather/:id", async (req: Request, res) => {
  try {
    const { id } = req.params;
    if (await verifId(id)) {
      deleteGroupFather(id);
